...
export function CharacterCreator({ onBack, onSave }) {
  ...
  const handleNext = () => {
    if (step === 1 && selectedRace) {
      ...
      const newHero = {
        name: "New Hero", // Nombre temporal por ahora
        race: selectedRace.name,
        class: selectedClass.name,
        stats: { str: 10, dex: 10, con: 10, int: 10, wis: 10, cha: 10 }, // Stats base
      };

      // Enviamos los datos al 'Jefe' (App.jsx)
      onSave(newHero);
    }
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300 pb-24">
          ...
          CLASSES.map((cls) => (
            ...
            >
              ...
              <div className="text-sm text-stone-400 space-y-1">
                ...
                <p>
                  <span className="text-stone-500">Saves:</span>{" "}
                  {cls.saves.join(", ")}
                </p>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 w-full p-6 bg-gradient-to-t from-neutral-900 via-neutral-900 to-transparent">
        ...
        >
          {step === 2 ? "Finish (Por ahora)" : "Next Step"}{" "}
