...
export function Dashboard({ heroes, onNavigate, onCreate, onImport }) {
  const fileInputRef = useRef(null);
  const [isImporting, setIsImporting] = useState(false);

  // --- FUNCIÓN EXPORTAR (DESCARGAR JSON) ---
  const handleExport = () => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (Array.isArray(importedHeroes)) {
          onImport(importedHeroes); // Enviamos los datos a App.jsx
          alert(`Success! Loaded ${importedHeroes.length} heroes.`);
        } else {
          alert("Invalid file format. Expected a list of heroes.");
        }
      } catch (err) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {/* TARJETAS DE PERSONAJES EXISTENTES */}
        {heroes.map((hero) => (
          <div
            key={hero.id}
